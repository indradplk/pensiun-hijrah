...
const AccManagementForm = ({ show, handleClose, accData, userData }) => {
  ...
  const handleAcceptSubmit = async () => {
    setLoading(true);
    try {
      await axios.put(`${dataServer.href}/api/v1/management/accept/${accData.id}`, {
        nama: accData.nama,
  ...
  return (
    <Modal show={show} onHide={handleClose} centered>
      <Modal.Header closeButton>
        <FormH1>Setujui Manajemen</FormH1>
      </Modal.Header>
      <Modal.Body>
        <Form>
...
export default AccManagementForm;
