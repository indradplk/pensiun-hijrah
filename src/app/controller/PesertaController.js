...
exports.unblockAccount = async (req, res) => {
    try {
        ...
        await Peserta.update({ status: true, block_count: 0, userUpdate }, { where: { no_peserta: no_peserta } });

        await ActivityAdmin.create({
            nik: userUpdate,
            log: `${nama} (${userUpdate}) membuka blokir akun peserta ${no_peserta}`
        });
...
exports.resetPassword = async (req, res) => {
    try {
        ...
        await Peserta.update({ password: hash, userUpdate }, { where: { no_peserta: no_peserta } });

        await ActivityAdmin.create({
            nik: userUpdate,
            log: `${nama} (${userUpdate}) mereset password akun peserta ${no_peserta}`
        });
