...
const AccAwardForm = ({ show, handleClose, accData, userData }) => {
  ...
  const handleAcceptSubmit = async () => {
    setLoading(true);
    try {
      await axios.put(`${dataServer.href}/api/v1/award/accept/${accData.id}`, {
  ...
  return (
    <Modal show={show} onHide={handleClose} centered>
      <Modal.Header closeButton>
        <FormH1>Setujui Penghargaan</FormH1>
      </Modal.Header>
      <Modal.Body>
        <Form>
