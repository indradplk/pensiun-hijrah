...
const Riwayat = ({ userData }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const transaksiResponse = await axios.get(`${dataServer.href}/api/v1/ppukp/transaksi/${userData.userId}?start_date=${startDate}&end_date=${endDate}`);
      setState((prevState) => ({
        ...prevState,
        data: transaksiResponse.data
      }));

      const saldoResponse = await axios.get(`${dataServer.href}/api/v1/ppukp/saldo/${userData.userId}?end_date=${startDate}`);
