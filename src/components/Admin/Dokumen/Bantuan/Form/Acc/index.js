...
const AccBantuanForm = ({ show, handleClose, accData, userData }) => {
  ...
  const handleAcceptSubmit = async () => {
    setLoading(true);
    try {
      await axios.put(`${dataServer.href}/api/v1/bantuan/accept/${accData.id}`, {
        title: accData.title,
        nama: userData.nama,
  ...
  return (
    <Modal show={show} onHide={handleClose} centered>
      <Modal.Header closeButton>
        <FormH1>Setujui Dokumen Bantuan</FormH1>
      </Modal.Header>
      <Modal.Body>
        <Form>
