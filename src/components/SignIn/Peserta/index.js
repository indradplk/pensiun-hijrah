...
const SignIn = () => {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      const response = await axios.post(process.env.REACT_APP_API_BASE_URL + '/peserta/login', {
        username,
        password,
        role: 'peserta'
      });
      ...
      // Cek apakah role adalah peserta
      if (content.role !== 'peserta') {
        setError('Hanya peserta yang dapat login!');
        setLoading(false);
        return;
      }
