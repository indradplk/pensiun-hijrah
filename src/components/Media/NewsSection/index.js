...
const NewsSection = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${dataServer.href}/api/v1/news?status=true`);
        const formattedData = response.data.map(item => ({
          ...item,
          kategori: convertKategori(item.kategori),
          createdAt: format(new Date(item.createdAt), 'dd MMM yyyy', { locale: id }), // Format tanggal
        }));
