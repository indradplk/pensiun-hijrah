...
const AddReportForm = ({ show, handleClose, addData, handleAddInputChange }) => {
  ...
  const handleAddSubmit = async () => {
    ...
    formData.append('path_report', addData.path_report);

    try {
      const response = await axios.post(process.env.REACT_APP_API_BASE_URL + '/report', formData, {
        headers: {
  ...
  useEffect(() => {
    setFormData({
      title: addData.title,
      path_report: addData.path_report,
    ...
    <Modal show={show} onHide={handleClose} centered>
      ...
      <Modal.Body>
        <Form>
          ...
          <FormInput id="path_report" type="file" name="path_report" onChange={handleAddInputChange} />
          {formData.path_report && (
            <p>{formData.path_report.name}</p>
          )}
