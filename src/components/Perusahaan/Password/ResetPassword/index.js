...
const ResetPasswordPerusahaan = ({ userData }) => {
  ...
  const fetchData = () => {
    axios
      .get(process.env.REACT_APP_API_BASE_URL + `/ppukp/perusahaan/${userData.username}`, {
        headers: {
  ...
  const fetchUserData = () => {
    axios
      ...
      .then((response) => {
        setEmail(response.data.content.perusahaanEmail);
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        process.env.REACT_APP_API_BASE_URL + `/perusahaan/${userData.username}`,
        {
          email: email,
