...
const AddManagementForm = ({ show, handleClose, addData, handleAddInputChange, userData }) => {
  ...
  const handleAddSubmit = async () => {
    ...
    data.append('namaAdmin', userData.nama);
    data.append('userUpdate', userData.nik);

    try {
      const response = await axios.post(`${dataServer.href}/api/v1/management`, data, {
        headers: {
