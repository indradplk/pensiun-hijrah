...
const RegistrasiItemSection = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${dataServer.href}/api/v1/registrasi/${id}`); // Gunakan ID dari URL
        const formattedData = {
          ...response.data,
          jenis_identitas: convertJenisIdentitas(response.data.jenis_identitas),
          warganegara: convertWarganegara(response.data.warganegara),
          tanggal_lahir: isValid(new Date(response.data.tanggal_lahir)) ? format(new Date(response.data.tanggal_lahir), 'dd MMMM yyyy', { locale: localeID }) : '',
          tanggal_lahir_ahli_waris_1: isValid(new Date(response.data.tanggal_lahir_ahli_waris_1)) ? format(new Date(response.data.tanggal_lahir_ahli_waris_1), 'dd MMMM yyyy', { locale: localeID }) : '',
          tanggal_lahir_ahli_waris_2: isValid(new Date(response.data.tanggal_lahir_ahli_waris_2)) ? format(new Date(response.data.tanggal_lahir_ahli_waris_2), 'dd MMMM yyyy', { locale: localeID }) : '',
          tanggal_lahir_ahli_waris_3: isValid(new Date(response.data.tanggal_lahir_ahli_waris_3)) ? format(new Date(response.data.tanggal_lahir_ahli_waris_3), 'dd MMMM yyyy', { locale: localeID }) : '',
          jenis_kelamin: convertJenisKelamin(response.data.jenis_kelamin),
          jenis_kelamin_ahli_waris_1: convertJenisKelamin(response.data.jenis_kelamin_ahli_waris_1),
          jenis_kelamin_ahli_waris_2: convertJenisKelamin(response.data.jenis_kelamin_ahli_waris_2),
          jenis_kelamin_ahli_waris_3: convertJenisKelamin(response.data.jenis_kelamin_ahli_waris_3),
          perkawinan: convertPerkawinan(response.data.perkawinan),
          peserta_pengalihan: response.data.peserta_pengalihan ? 'Ya' : 'Tidak',
          peserta_dapen: response.data.peserta_dapen ? 'Ya' : 'Tidak',
          pembayaran_iuran: convertBayar(response.data.pembayaran_iuran)
        };
        setRegistrasiItem(formattedData);
      } catch (error) {
        console.error('Error fetching Registrasi item:', error);
      }
    };
  ...
  useEffect(() => {
    axios.get(`${dataServer.href}/api/v1/master/agama`)
      .then(response => {
        setAgama(response.data);
      })
      .catch(error => console.error('Error fetching agama:', error));
  }, []);
  ...
  useEffect(() => {
    axios.get(`${dataServer.href}/api/v1/master/pendidikan`)
        .then(response => {
            setPendidikan(response.data);
        })
        .catch(error => console.error('Error fetching pendidikan:', error));
  }, []);
  ...
  useEffect(() => {
    axios.get(`${dataServer.href}/api/v1/master/sampingan`)
      .then(response => {
        setSampingan(response.data);
      })
      .catch(error => console.error('Error fetching penghasilan:', error));
  }, []);
  ...
  useEffect(() => {
    axios.get(`${dataServer.href}/api/v1/master/dana`)
      .then(response => {
        setDana(response.data);
      })
      .catch(error => console.error('Error fetching sumber dana:', error));
  }, []);
  ...
  const handleDownload = (path) => {
    window.open(`${dataServer.href}/peserta/registrasi/${path}`, '_blank');
  };
