...
const ResetPasswordPeserta = ({userData}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response1 = await axios.get(`${dataServer.href}/api/v1/peserta/${no_peserta}`);
      const response2 = await axios.get(`${dataServer.href}/api/v1/ppip/peserta/${no_peserta}`);
