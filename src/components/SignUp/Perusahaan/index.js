...
const SignUpSection = () => {
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        const response = await axios.post(`${dataServer.href}:4000/api/v1/perusahaan/auth`, {}, { withCredentials: true });
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      const response = await axios.post(`${dataServer.href}:4000/api/v1/perusahaan/register`, {
